...
function fixData(badData) {
  return badData.map(user => {
    const [id, name, surname, email, age, cash] = user; // destrukturyzacja
...
console.log("Suma pieniędzy:", totalCash);
const avgAge = fixedUsers.reduce((sum, user) => sum + user.age, 0) / fixedUsers.length;
console.log("Średni wiek:", avgAge);
